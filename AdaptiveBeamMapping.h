...
public :
        ...
        /** Creation of AdaptativeBeamMapping
	  * The creation of AdaptativeBeamMapping require to find a StateInput, StateOutPut and a Interpolation
	  * Generally, StateInput and Interpolation are placed in the higher context, StateOutPut is placed on the same context
          *

	template<class T>
	static bool canCreate(T* obj, sofa::core::objectmodel::BaseContext* context, sofa::core::objectmodel::BaseObjectDescription* arg)
	{
                BeamInterpolation<In>* _interpolation = NULL;

                _interpolation = sofa::core::objectmodel::ObjectRef::parse< BeamInterpolation<In> > ( "interpolation", arg);

                // If attribute "interpolation" is not defined on the node, we will search the interpolation on the higher level context
                if (_interpolation == NULL)
                        context->get(_interpolation, sofa::core::objectmodel::BaseContext::SearchUp);
        ...
        static typename T::SPtr create(T*& tObj, core::objectmodel::BaseContext* context, core::objectmodel::BaseObjectDescription* arg)
	{
		...
		if(arg){
			...
			//find Interpolation by the path "interpolation", or on the same level of InputState
                        if(arg->getAttribute("interpolation",NULL) != NULL){
                                _interpolation = sofa::core::objectmodel::ObjectRef::parse< BeamInterpolation<TIn> >("interpolation", arg);
                        }
                        else{
                                _stin->getContext()->get(_interpolation);
                                context->serr << "WARNING ("<<className ( tObj ) <<") : interpolation in used : <"<< _interpolation->getClassName() <<"/>  named  "<< _interpolation->getName() <<context->sendl;
                        }
                }

		//Creation and adding on scene
                typename T::SPtr obj = sofa::core::objectmodel::New<T>(_stin,_stout,NULL,false);
