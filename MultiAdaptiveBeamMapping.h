...
public :
	...
	/** Creation of UnifiedMultiAdaptativeBeamMapping
	  * The creation of UnifiedMultiAdaptativeBeamMapping require to find a StateInput, StateOutPut and a InterventionalRadiologyController
	  * Generally, StateInput and InterventionalRadiologyController are placed in the higher context, StateOutPut is placed on the same context
          *

	template<class T>
	static bool canCreate(T* obj, sofa::core::objectmodel::BaseContext* context, sofa::core::objectmodel::BaseObjectDescription* arg)
	{
		InterventionalRadiologyController<TIn>* _controller = NULL;

		_controller = sofa::core::objectmodel::ObjectRef::parse< InterventionalRadiologyController<TIn> > ( "controller", arg);

		// If attribute "controller" is not defined on the node, it will searched the on the higher level context
		if (_controller == NULL)
			context->get(_controller, sofa::core::objectmodel::BaseContext::SearchUp);
        ...
        static typename T::SPtr create(T*& tObj, core::objectmodel::BaseContext* context, core::objectmodel::BaseObjectDescription* arg)
	{
		...
		core::State<In>*                             _stin = NULL;
		core::State<Out>*                           _stout = NULL;
		InterventionalRadiologyController<In>* _controller = NULL;

		if(arg){
			//find input State by the path "input", or on the higher level context if "input" is not given
			if(arg->getAttribute("input",NULL) != NULL){
				_stin = sofa::core::objectmodel::ObjectRef::parse< core::State<In> >("input", arg);
			}
			else{
				if(BaseObject* m_baseObj = dynamic_cast<BaseObject*> ( arg->findObject("../..") ) )
					_stin = dynamic_cast<core::State<TIn>*> (m_baseObj);
				else if(BaseContext* m_basecontext = dynamic_cast<BaseContext*> ( arg->findObject("../..") ) )
					_stin = dynamic_cast<core::State<TIn>*> (m_basecontext->getState());
			}

			//find output State by the path "output", or on the same context if "output" is not given
			if(arg->getAttribute("output",NULL) != NULL){
				_stout = sofa::core::objectmodel::ObjectRef::parse< core::State<TOut> >("output", arg);
			}
			else{
				if(BaseObject* m_baseObj = dynamic_cast<BaseObject*> ( arg->findObject("..") ) )
					_stout = dynamic_cast<core::State<Out>*> (m_baseObj);
				else if(BaseContext* m_basecontext = dynamic_cast<BaseContext*> ( arg->findObject("..") ) )
					_stout = dynamic_cast<core::State<Out>*> (m_basecontext->getState());
			}

			//find Interpolation by the path "controller", or on the same level of InputState
			if(arg->getAttribute("controller",NULL) != NULL){
				_controller = sofa::core::objectmodel::ObjectRef::parse< InterventionalRadiologyController<TIn> >("controller", arg);
			}
			else{
				_stin->getContext()->get(_controller);
                                context->serr << "WARNING ("<<className ( tObj ) <<") : controller in used : <"<< _controller->getClassName() <<"/>  named  "<< _controller->getName() <<context->sendl;
			}
		}


                //Creation and adding on scene
                typename T::SPtr obj = sofa::core::objectmodel::New<T>(_stin,_stout,_controller);
